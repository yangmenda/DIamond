...
public class Parser {
	...
	protected static class OrTree extends Element
	{
		...
		protected void insert(Parser p)throws ParseException
		{
	...
	protected static abstract class AToken extends Element
	{
		...
		protected AToken(Class<?extends ASTLeaf>type) {
			if(type==null)type=ASTLeaf.class;
	...
	protected static class Expr extends Element{
		...
		{
			Token t=lexer.peek(0);
			if(t.isIdentifier())return ops.get(t.getText());
			else return null;
		}
	...
	public Parser insertChoice(Parser p)throws ParseException
	{
		Element e=elements.get(0);
